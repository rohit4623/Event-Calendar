...
require_once "functions_pdf.inc";


function genDateSelectorforPdf($prefix, $day, $month, $year, $form_id = '', $disabled = FALSE)
{
	...
	foreach ($report_search_field_order as $key) {
		switch ($key) {
			case 'report_start':
				...
				genDateSelectorforPdf("from_", $vars['from_day'] = '1', $vars['from_month'] = '10', $vars['from_year'] = '2017');
				echo "</div>\n";
				break;
			...
			case 'report_end':
				...
				genDateSelectorforPdf("to_", $vars['to_day'], $vars['to_month'], $vars['to_year']);
				echo "</div>\n";
				break;
...
function generate_exportpdf_button()
{
...
function type_wrap($string, $data_type)
{
	...
	if ($output_format == OUTPUT_HTML || $output_format == OUTPUT_PDF) {
...
function open_report()
{
	...
	if (($output_format == OUTPUT_HTML || $output_format == OUTPUT_PDF) && !$ajax) {
		// echo "<div id=\"report_output\" class=\"datatable_container\">\n";
		// echo "<table class=\"admin_table display\" id=\"report_table\">\n";

		$GLOBALS['html'] .= "<div id=\"report_output\" class=\"datatable_container\">";
		$GLOBALS['html'] .= "<table class=\"admin_table display\" id=\"customers report_table\" cellspacing=\"0\" cellpadding=\"1\" border=\"1\">";
...
function close_report()
{
	...
	} elseif ($output_format == OUTPUT_HTML || $output_format == OUTPUT_PDF) {
		//  echo "</table>\n";
		//  echo "</div>\n";
...
function output_head_rows(&$rows, $format)
{
	...
	if ($format == OUTPUT_HTML || $output_format == OUTPUT_PDF) {
		// echo "<colgroup>";
		$GLOBALS['html'] .= "<colgroup>";
...
function output_foot_rows(&$rows, $format)
{
	...
	echo ($format == OUTPUT_HTML || $output_format == OUTPUT_PDF) ? "<tfoot>\n" : "";
	foreach ($rows as $row) {
		output_row($row, $format, FALSE);
	}
	echo ($format == OUTPUT_HTML || $output_format == OUTPUT_PDF) ? "</tfoot>\n" : "";
...
function report_row(&$rows, &$data)
{
	...
	if (($output_format == OUTPUT_HTML || $output_format == OUTPUT_PDF) && $ajax_capable && !$ajax) {
		return;
	}
		...
		switch ($field) {
			...
			case 'approval_enabled':
				...
				if ($data['approval_enabled']) {
					...
					$value = ($data['status'] & STATUS_COMPLETE) ? "Update pending" : $value;
				} else {
					$value = '';
				}
				break;
		...
		&$room_hash, &$name_hash)
{
	...
	$room .= ($output_format == OUTPUT_HTML || $output_format == OUTPUT_PDF) ? "<br>" : '/';
	...
	foreach ($name_hash as $name => $is_present) {
		...
		foreach ($room_hash as $room => $mode) {
			if (isset($count[$room][$name])) {
				...
				$cells[] = ($output_format == OUTPUT_HTML || $output_format == OUTPUT_PDF) ? "&nbsp;" : '';
				$cells[] = ($output_format == OUTPUT_HTML || $output_format == OUTPUT_PDF) ? "&nbsp;" : '';
			} 
		}
		// Add the total column(s) onto the end
...
foreach ($fields as $field) {
	$key = $field['name'];
	if (!in_array($key, $standard_fields['entry']) && in_array($key, $is_report_criteria)) {
		$custom_fields[$key] = '';
...
$field_order_list = array('name', 'area_name', 'room_name');
if($output_format != OUTPUT_PDF) {
	$field_order_list[] = 'start_time';
	$field_order_list[] = 'end_time';
}
$field_order_list[] = 'description';
$field_order_list[] = 'type';
$field_order_list[] = 'create_by';
$field_order_list[] = 'approval_enabled';
foreach ($custom_fields as $key => $value) {
...
$output_form = (($output_format == OUTPUT_HTML || $output_format == OUTPUT_PDF) && !$ajax && !$cli_mode) ||
...
if ($output_form) {
	...
	echo "<form class=\"form_general\" id=\"report_form\" method=\"get\" action=\"generatepdf.php\">\n";
	echo "<fieldset>\n";
...
if ($phase == 2) {
	if (($nmatch == 0) && !$cli_mode && ($output_format == OUTPUT_HTML || $output_format == OUTPUT_PDF)) {
		if ($ajax) {
			// header("Content-Type: application/json");
			echo json_encode($json_data);
		} else {
			//echo "<p class=\"report_entries\">" . get_vocab("nothing_found") . "</p>\n";
			$GLOBALS['html'] .= "<p class=\"report_entries\">" . get_vocab("nothing_found") . "</p>\n";
		}
		unset($res);
		...
		if ($output == REPORT) {
			...
			if($output_format == OUTPUT_HTML) {
				$GLOBALS['html'] .= "<br>";
				echo "<br>".$GLOBALS['html']. "<br>";
				echo "<br>";
			} 
			elseif($output_format == OUTPUT_PDF) {
